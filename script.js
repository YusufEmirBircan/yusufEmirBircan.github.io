...
let particles = [];
const mouse = { x: null, y: null, radius: 100 }; // Mobilde biraz daha küçük yarıçap
...
window.addEventListener('touchstart', (event) => {
    // Scroll olayını engellemek için parmak dokunduğunda radius arttırılabilir veya preventDefault denenebilir
...
function getTextPoints(text) {
    ...
    // Mobilde kalite için daha sık, PC'de performans için normal tarama
...
class Particle {
    constructor(targetX, targetY) {
        ...
        // Başlangıçta ekranın her yerine rastgele dağılmış halde (mantıksal koordinatlar)
        this.x = Math.random() * window.innerWidth;
        this.y = Math.random() * window.innerHeight;
        this.size = Math.random() * 0.7 + 0.4; // Dengeli boyut (0.4 - 1.1)
...
window.addEventListener('resize', () => {
    // Mobil tarayıcılarda aşağı kaydırınca URL bar gizlendiği için height değişir.
    // Animasyonun bozulmaması için sadece genişlik (width) anlamlı değişirse init yapıyoruz.
